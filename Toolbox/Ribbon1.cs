    ...
    public partial class Ribbon1 : IRibbonExtensibility
    {
        ...
        public void ExportProperties(IRibbonControl control)
        {
            ...
            if (fileDialog.Show() == -1)
            {
                ...
                if (Settings.Default.IncludeVariables)
                {
            ...
            if (fileDialog.Show() == -1)
            {
                ...
                foreach (string line in File.ReadLines(filename))
                {
                    try
                    {
                        ...
                        string val = result[1];
                        PropertyFunctions.SetDocumentValue(name, val);
        ...
        public void OpenPropertyUpdater(IRibbonControl control)
        {
            ...
            propertyUpdater.itemType.Items.Add("Document Property");
            if (Settings.Default.IncludeVariables)
            {
                propertyUpdater.itemType.Items.Add("Document Variable");
            }
            if (Settings.Default.IncludeBookmarks)
            {
                propertyUpdater.itemType.Items.Add("Bookmarks");
            }
